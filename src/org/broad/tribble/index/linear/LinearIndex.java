...
public class LinearIndex extends AbstractIndex implements Index {
    ...
    public boolean isCurrentVersion() {
        ...
        for ( org.broad.tribble.index.ChrIndex chrIndex : chrIndeces.values() )
    ...
    public LinearIndex(List<ChrIndex> indicies, File inputFile) {
        ...
        for (ChrIndex index : indicies)
            chrIndeces.put(index.getName(),index);
    }
    ...
    public LinkedHashSet<String> getSequenceNames() {
        return (chrIndeces == null ? new LinkedHashSet<String>() : new LinkedHashSet<String>(chrIndeces.keySet()));
    }
    ...
    public static class ChrIndex implements org.broad.tribble.index.ChrIndex {
        ...
        private static ChrIndex mergeBlocks(ChrIndex idx) {
            ...
            if ( ! blocks.hasNext() ) throw new IllegalStateException("Block iterator cannot be empty at the start for " + idx.getName());
    ...
    public Index optimize(double threshold) {
        if ( enableAdaptiveIndexing ) {
            ...
            List<ChrIndex> newIndices = new ArrayList<ChrIndex>(this.chrIndeces.size());
            for ( String name : chrIndeces.keySet() ) {
                LinearIndex.ChrIndex oldIdx = (LinearIndex.ChrIndex)chrIndeces.get(name);
    ...
    public void writeTable(PrintStream out) {
        ...
        for ( String name : chrIndeces.keySet() ) {
            LinearIndex.ChrIndex chrIdx = (LinearIndex.ChrIndex)chrIndeces.get(name);
            int blockCount = 0;
