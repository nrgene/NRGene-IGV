 ...
 * @author jrobinso
 */
public class FeatureTrack extends AbstractTrack implements ITrackBlockHeight {

    private static Logger log = Logger.getLogger(FeatureTrack.class);

    public static final int MINIMUM_FEATURE_SPACING = /*5 DK back to 1.x value*/ 1;
    ...
    private static final int DEFAULT_EXPANDED_MIN_HEIGHT = 10;
    private static final int DEFAULT_EXPANDED_HEIGHT = 35;
    private static final int DEFAULT_SQUISHED_HEIGHT = 12;
    private static final boolean DEFAULT_SORTABLED = true;	// DK this is reversed from the generic version;

    // DK 
    private int expandedMinRowHeight = DEFAULT_EXPANDED_MIN_HEIGHT;
    ...
    private static final int EXTEND_FACTOR = 1;
    private static final int MAX_WINDOW_END = 1000*1000*1000;
    ...
    */
    ...
    private String	tooltipAttributes;
    ...
    private int trackBlockHeight = 0;		// 0 means default
    ...
    protected void init(FeatureSource source) {
        ...
        if (source.getFeatureWindowSize() > 0) {
            super.setVisibilityWindow(source.getFeatureWindowSize());
        }
        
        GFFParser.registerTrackSource(this, source);
    }
    ...
    public int getHeight() {
        ...
        int rowHeight = getDisplayMode() == DisplayMode.SQUISHED ? squishedRowHeight : expandedMinRowHeight;
    ...
    public String getValueStringAt(String chr, double position, int y, ReferenceFrame frame) {
    	return getValueStringAt(chr, position, y, frame, IGV.getInstance().getGenomeManager().currentGenome);
    
    }
    public String getValueStringAt(String chr, double position, int y, ReferenceFrame frame, Genome genome) {

    	log.debug("getValueStringAt: track: " + this + ", chr: " + chr + ", position: " + position + ", y: " + y + ", frame: " + frame);
    	
    	String 	result = getValueStringAtBody(chr, position, y, frame, genome);
    ...
    public String getValueStringAtBody(String chr, double position, int y, ReferenceFrame frame) {
    	return getValueStringAtBody(chr, position, y, frame, null);
    }
    
    public String getValueStringAtBody(String chr, double position, int y, ReferenceFrame frame, Genome genome) {
    	...
    	*/
        ...
        if (showFeatures) {
            ...
            // DK - filter out features which started exactly on the position (???) --- THIS IS NO LONGER REQUIRED! or at least on generic maps
            if ( genome != null && !genome.isGeneticMap() )
            {
	            List<Feature> dkFeatures = new LinkedList<Feature>(allFeatures);
	            allFeatures.clear();
	            for ( Feature feature : dkFeatures )
	            {
	            	if ( feature.getStart() != (int)position )
            ...
            for (Feature feature : allFeatures) {
                if (feature != null && feature instanceof IGVFeature) {
                    ...
                    String vs = igvFeature.getValueString(position, null, genome);
                    if (!firstFeature) {
                        buf.append("<br>--------------<br>");
                    }
                    buf.append(vs);
            ...
            if ( tooltipAttributes != null && tooltipAttributes.length() > 0 )
            {
            	...
            	for ( String name : tooltipAttributes.split(",") )
            		names.add(name.trim());
            	
            	// break into lines, check start of line
            	StringBuilder	sb = new StringBuilder();
            	for ( String line : buf.toString().split("<br>") )
            	{
            		line = line.trim();
            		int			index = line.indexOf(" = ");
            		if ( index < 0 )
            			sb.append(line + "<br>");
            		else if ( names.contains(line.substring(0, index)) )
            			sb.append(line + "<br>");
            	}
    ...
    protected List<Feature> getAllFeatureAt(String chr, double position, int y, ReferenceFrame frame) {
        ...
        if (features != null) {
            ...
            feature = FeatureUtils.getAllFeaturesAt(position, maxFeatureLength, minWidth, features, true, true);
        }
        return feature;
    }
    ...
    public boolean handleDataClick(TrackClickEvent te) {
        ...
        if ( referenceFrame != null && e.getClickCount() == 1 && (e.isShiftDown() || e.isMetaDown()) /*&& e.isAltDown()*/ && !e.isPopupTrigger() && e.getButton() == MouseEvent.BUTTON1) 
        {
        	...
        	String		description = getValueStringAt(referenceFrame.getChrName(), displayLocation, e.getY(), referenceFrame, genome);
            ...
            sb.append(genome.getLocationFormatter().format(displayLocation));
            sb.append("<br><br>");
    ...
    {
    	log.debug("e" + e.isAltDown());
    	log.debug("e" + e.isAltGraphDown());
    	log.debug("e" + e.isConsumed());
    	log.debug("e" + e.isControlDown());
    	log.debug("e" + e.isMetaDown());
    	log.debug("e" + e.isPopupTrigger());
    	log.debug("e" + e.isShiftDown());
	}
    ...
    protected void renderCoverage(RenderContext context, Rectangle inputRect) {
        ...
        List<LocusScore> scores = source.getCoverageScores(context.getChr(), (int) context.getOrigin(),
                (int)Math.ceil(context.getEndLocation()), context.getZoom());
        if (scores == null) {
            ...
            // DK - block max from being 0 since this will yield the range 0->0
            if ( max == 0.0 )
            	max = 1;
        ...
        NamedRunnable runnable = new NamedRunnable() {
            public void run() {
                ...
                try {
                    ...
                    // Determine [expandedStart,expandedEnd] - the boundaries of the window for features loading
                    long delta = ((long)end - (long)start) / 2;
                    delta *= EXTEND_FACTOR;

                    long expandedStartLong = Math.max(0,start - delta);
                    long expandedEndLong = Math.min(MAX_WINDOW_END, end + delta);
      
                    Genome genome = IGV.getInstance().getGenomeManager().getCurrentGenome();
                    if (genome != null) {
                        Chromosome c = genome.getChromosome(chr);
                        if (c != null) expandedEndLong = Math.min(c.getLength()+1, expandedEndLong);
                    }
                 
                    int expandedStart = (int) expandedStartLong;
                    int expandedEnd = (int) expandedEndLong;
    ...
    public void setVisibilityWindow(int windowSize) {
        ...
        source.setFeatureWindowSize(getVisibilityWindow());
        if ( !ApiRequest.isNiu() )
        	super.setVisibilityWindow(source.getFeatureWindowSize());
        else
        {
        	if ( source.getFeatureWindowSize() > 1 )
            	super.setVisibilityWindow(source.getFeatureWindowSize());
        }
    }
    ...
    public void restorePersistentState(Map<String, String> attributes) {
        ...
        */
        ...
        value = attributes.get("tooltipAttributes");
        if (value != null) {
        	tooltipAttributes = value;
        }
    ...
    public Map<String, String> getPersistentState() {
        ...
        /*
        if ( showTooltip )
        	stateMap.put("showTooltip", String.valueOf(showTooltip));
        */
        if ( showTrackSeparator )
        	stateMap.put("showTrackSeparator", String.valueOf(showTrackSeparator));
        if ( tooltipAttributes != null )
        	stateMap.put("tooltipAttributes", tooltipAttributes);
	...
	*/
	...
	public String getTooltipAttributes() {
		return tooltipAttributes;
	}

	public void setTooltipAttributes(String tooltipAttributes) {
		this.tooltipAttributes = tooltipAttributes;
	}
	...
	public void removed() 
	{
		...
		GFFParser.deregisterTrackSource(this, source);
	}

	public int getTrackBlockHeight() {
		return trackBlockHeight;
	}

	public void setTrackBlockHeight(int trackBlockHeight) {
		this.trackBlockHeight = trackBlockHeight;
	}
