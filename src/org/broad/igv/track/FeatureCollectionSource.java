...
public class FeatureCollectionSource implements FeatureSource {
    ...
    class CoverageDataSource extends AbstractDataSource {
        ...
        private DataTile computeCoverage(String chr, double start, double end) {

        	// overriding windowSize here ...
        	int			nBins = Globals.COVERAGE_BINS_OVERRIDE;
        	double		windowSize = (end - start) / nBins;
            int[] starts = new int[nBins];
            int[] ends = new int[nBins];
            double[] startFractions = new double[nBins];
            double[] endFractions = new double[nBins];
            
            for (int i = 0; i < nBins; i++) {
            	double binStart = start + i * windowSize;
            	double binEnd = start + (i + 1) * windowSize;
            	
                starts[i] = (int)binStart;
                ends[i] = (int)binEnd;
                
                startFractions[i] = binStart - starts[i];
                endFractions[i] = binEnd - ends[i];
            }
            ...
            if (features != null) {
                for (Feature f : features) {
                	...
                	double		featureStart = f.getStart();
                	double		featureEnd = f.getEnd();
                	if ( f instanceof BasicFeature )
                	{
                		BasicFeature		bf = (BasicFeature)f;
                		
                		featureStart += bf.getStartFraction();
                		featureEnd += bf.getEndFraction();
                	}
                	
                    int startBin = (int)(featureStart / windowSize);
                    int endBin = (int)(featureEnd / windowSize);
                    for (int i = startBin; i < endBin; i++) {
                        values[i] = values[i] + 1;
                        dataMax = Math.max(dataMax, values[i]);
                    }
            ...
            return new DataTile(starts, ends, values, null, startFractions, endFractions);
        ...
        protected void computeGenomeCoverage() {
            ...
            Genome currentGenome = IGV.getInstance().getGenomeManager().getCurrentGenome();
            if ( currentGenome.isGeneticMap() )
            {
            	computeGenericMapGenomeCoverage();
            	return;
            }
        ...
        protected void computeGenericMapGenomeCoverage() {
            ...
            double step = ((double) currentGenome.getLength() / 1000.0) / nBins;
            for (int i = 0; i < nBins; i++) {
                starts[i] = (int) (i * step);
                ends[i] = (int) ((i + 1) * step);
                startFractions[i] = (i * step) - starts[i];
                endFractions[i] = ((i + 1) * step) - ends[i];
            }
            ...
            for (String chr : currentGenome.getChromosomeNames()) {
                ...
                if (features != null) {
                    ...
                    for (Feature f : features) {
                        ...
                        double genStart = ((offset + featureStart) / 1000);
                        double genEnd = ((offset + featureEnd) / 1000);
                        int binStart = Math.min(values.length - 1, (int) (genStart / step));
                        int binEnd = Math.min(values.length - 1, (int) (genEnd / step));
                        for (int i = binStart; i <= binEnd; i++) {
                            values[i] = values[i] + 1;
            ...
            dataCache.put(Globals.CHR_ALL, new DataTile(starts, ends, values, null, startFractions, endFractions));
        }
