...
import org.broad.igv.variant.TrackedObject;
...
public abstract class AbstractTrack implements Track {
    ...
    public void renderAttributes(Graphics2D graphics, Rectangle trackRectangle, Rectangle visibleRect,
        ...
        for (String name : names) {
            ...
            if (attributeValue != null) {
                ...
                Color 		color = AttributeManager.getInstance().getColor(key, attributeValue);
    ...
    public void setAttributeValue(String name, String value) {
        ...
        AttributeManager.getInstance().addAttributeName(name, null);
    }
    ...
    public String getAttributeValue(String attributeName) {
        ...
        if (value == null) {
            ...
            // consult soft attributes
            if ( value == null )
            	value = SoftAttributeManager.getInstance().getAttribute(this, attributeName);
    ...
    public Map<String, String> getPersistentState() {
        ...
        if ( this instanceof ITrackBlockHeight )
        {
        	int		blockHeight = ((ITrackBlockHeight)this).getTrackBlockHeight();
        	if ( blockHeight >= 0 )
        	{
                String value = Integer.toString(blockHeight);
                attributes.put(SessionReader.SessionAttribute.BLOCK_HEIGHT.getText(), value);
        	}
        }
        ...
        // color
        if (posColor != null) {
            attributes.put(SessionReader.SessionAttribute.COLOR.getText(), colorAsString(posColor));
        }
        if (altColor != null) {
            attributes.put(SessionReader.SessionAttribute.ALT_COLOR.getText(), colorAsString(altColor));
        }
        ...
        if ( getNameColor() != null && !getNameColor().equals(Color.BLACK) )
        	attributes.put("nameColor", colorAsString(getNameColor()));
        if ( getNameFont() != null )
        	attributes.put("nameFont", fontAsString(getNameFont()));
    ...
    public void restorePersistentState(Map<String, String> attributes) {
        ...
        if ( this instanceof ITrackBlockHeight )
        {
            String blockHeight = attributes.get(SessionReader.SessionAttribute.BLOCK_HEIGHT.getText());
            if ( blockHeight != null )
                try {
                    ((ITrackBlockHeight)this).setTrackBlockHeight(Integer.parseInt(blockHeight));
                } catch (NumberFormatException e) {
                    log.error("Error restoring track block height: " + blockHeight);
                }
        }
        ...
        String nameFontString = attributes.get("nameFont");
        if (nameFontString != null) {
            try {
                setNameFont(fontFromString(nameFontString));
            } catch (NumberFormatException e) {
                log.error("Error restoring name font: " + nameFontString);
            }
        }  
        
        String nameColorString = attributes.get("nameColor");
        if (nameColorString != null) {
            try {
                setNameColor(colorFromString(nameColorString));
            } catch (NumberFormatException e) {
                log.error("Error restoring name color: " + nameColorString);
            }        	
        }
        ...
        if (colorString != null) {
            try {
            	posColor = colorFromString(colorString);
            } catch (NumberFormatException e) {
        ...
        if (altColorString != null) {
            try {
            	altColor = colorFromString(altColorString);
            } catch (NumberFormatException e) {
                log.error("Error restoring color: " + colorString);
            }
        }
	...
	private String colorAsString(Color color)
	{
	...
	private String fontAsString(Font font)
	{
		StringBuilder	sb = new StringBuilder();
	...
	private Font fontFromString(String text)
	{
		...
		boolean			italics = font.isItalic();

		if ( toks.length > 0 )
			family = toks[0];
		if ( toks.length > 1 )
			size = Integer.parseInt(toks[1]);
		if ( toks.length > 2 )
			bold = Boolean.parseBoolean(toks[2]);
		if ( toks.length > 3 )
			italics = Boolean.parseBoolean(toks[3]);
		
		int				attrs = (bold ? Font.BOLD : 0) | (italics ? Font.ITALIC : 0);
		
		return FontManager.getFont(family, attrs, size, null);
	}
