...
public class CodecFactory {
	...
	public static class MyVCFCodec extends VCFCodec
	{
		boolean		autoEmpty;
		
		public MyVCFCodec(boolean autoEmpty)
		{
			super();
		...
		{
			if ( !autoEmpty )
				return super.readHeader(reader);
			
			Object		result;
			try 
			{
				result = super.readHeader(reader);
			} catch (TribbleException.InvalidHeader e) 
			{
				// canned header
				List<String> 		headerStrings = new LinkedList<String>();
				String				line;
				
				headerStrings.add("##fileformat=VCFv4.0");
     ...
     */
    public static FeatureCodec getCodec(String fullPath) {

    	String		path = !ApiRequest.isNiu() ? fullPath :ApiRequest.extractCleanPath(fullPath); 
        ...
        if (fn.endsWith(".vcf4")) {
            ...
            return new VCFWrapperCodec(getVCFCodec(path, fullPath));
     ...
     * @return
     */
    private static FeatureCodec getVCFCodec(String path, String fullPath) {
        ...
        try {
            ...
            if (path.toLowerCase().endsWith(".gz")) {
                reader = new BufferedReader(new InputStreamReader(new BlockCompressedInputStream(
                        SeekableStreamFactory.getStreamFor(fullPath))));
            } else {
                reader = ParsingUtils.openBufferedReader(fullPath);
            }
        ...
        if ( !ApiRequest.isNiu() )
        	...
        	return new MyVCFCodec(ApiRequest.isApiUrl(fullPath));
    }	
}
