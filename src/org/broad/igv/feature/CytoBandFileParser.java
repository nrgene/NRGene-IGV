...
public class CytoBandFileParser {
     ...
     * @return
     */
    public static LinkedHashMap<String, Chromosome> loadData(BufferedReader reader, long factor) {
        ...
        try {
            ...
            while ((nextLine = reader.readLine()) != null && (nextLine.trim().length() > 0)) {
                ...
                Cytoband cytoData = new Cytoband(chr);
                parseData(data, cytoData, factor);
    ...
    private static void parseData(String[] tokens, Cytoband cytoData, long factor) {

    	cytoData.setStart((int)(Long.parseLong(tokens[1].trim()) / factor));
    	cytoData.setEnd((int)(Long.parseLong(tokens[2].trim()) / factor));
