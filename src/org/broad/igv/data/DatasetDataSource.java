...
public class DatasetDataSource extends AbstractDataSource {
    ...
    protected DataTile getRawData(String chr, int startLocation, int endLocation) {
        ...
        if (chr.equals(Globals.CHR_ALL) && genomeSummaryData != null) {
            ...
            double[]		startFractions = null;
            double[]		endFractions = null;
            if ( genome.isGeneticMap() && (dataset instanceof WiggleDataset) )
            {
            	startFractions = ((WiggleDataset)dataset).getStartFractions(chr);
            	endFractions = ((WiggleDataset)dataset).getEndFractions(chr);
            }
            
            return new DataTile(startLocs, endLocs, data, features, startFractions, endFractions);            								
        }
    }
