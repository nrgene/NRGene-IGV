...
public class SortTracksMenuAction extends MenuAction {
    ...
    final public void doSortTrackByAttribute() {
        ...
        SortDialog dialog = new SortDialog(mainFrame.getMainFrame(), true, availableSortKeys, false);
        ...
        if (selectedSortKeys != null) {
        	...
        	AttributeHeaderPanel.reportSort(selectedSortKeys, dialog.isAscending());

        	if ( !IGVContentPane.isUseTabs() )  
        	{
        		IGV.getInstance().getTrackManager().sortAllTracksByAttributes(selectedSortKeys, dialog.isAscending(), false);
                mainFrame.getMainFrame().repaint();
        	}
        	else
        	{
        		for ( TrackManager trackManager : IGV.getInstance().getContentPane().tabsListTrackManagers() )
        			trackManager.sortAllTracksByAttributes(selectedSortKeys, dialog.isAscending(), false);
        	}
        }
        else
