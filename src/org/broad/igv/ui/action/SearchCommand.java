...
public class SearchCommand implements Command {
    ...
    public void execute() {
        ...
        // Feature search
            ...
            if (feature != null) {
                ...
                double 			featureStart = feature.getStart();
                double 			featureEnd = feature.getEnd();
                if ( genome != null && genome.isGeneticMap() && ((chromosome = genome.getChromosome(feature.getChr())) != null) )
                {
                	/* DK - set to a fixed 1cM 
                	int			size = chromosome.getLength();
                	
                	flankingRegion = size / 10;
                	*/
                	flankingRegion = 1;
                	
                    if ( feature instanceof AbstractFeature )
                    {
                    	featureStart += ((AbstractFeature)feature).getStartFraction();
                    	featureEnd += ((AbstractFeature)feature).getEndFraction();
                    }
                }
                
                
                int start = Math.max(0, (int)Math.floor(featureStart - flankingRegion));
                int end = (int)Math.ceil(feature.getEnd() + flankingRegion);
    ...
    private int[] getStartEnd(String posString) {
        try {
        	...
        	Genome		genome = IGV.getInstance().getGenomeManager().currentGenome;
        	boolean		fineGrained = (genome.getDecimalPlaces() > 0);
            ...
            if (posTokens.length == 2 && fineGrained )
            {
            	// keep it as is
            }
            else if (posTokens.length == 1 || (end - start) < 10) {
                int center = (start + end) / 2;
                start = center - 20;
                end = center + 20;
            } else {
