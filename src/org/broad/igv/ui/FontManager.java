...
public class FontManager {
    ...
    static public Font getFont(int attribute, int size) {
    	return getFont(attribute, size, null);
    }

    static public Font getFont(int attribute, int size, String scalePrefKey) {
    	return getFont(null, attribute, size, scalePrefKey);
    }
    
    static public Font getFont(String family, int attribute, int size, String scalePrefKey) {
        final PreferenceManager prefManager = PreferenceManager.getInstance();
        String fontFamily = family != null ? family : prefManager.get(PreferenceManager.DEFAULT_FONT_FAMILY);
        
        if ( scalePrefKey != null )
        {
        	float		scale = prefManager.getAsFloat(scalePrefKey);
        	size *= scale;
        }
