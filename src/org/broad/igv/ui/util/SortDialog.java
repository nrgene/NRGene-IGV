...
public class SortDialog extends javax.swing.JDialog {
    ...
    private boolean allTracksMode = false;
    private boolean	sortSingleTrack = false;

    /**
     * Creates new form SortDialog
     */
    public SortDialog(java.awt.Frame parent, boolean modal, Object[] data, boolean allTracksMode) {
        super(parent, modal);
        this.allTracksMode = allTracksMode;
        initComponents();
    ...
    private void initComponents() {
        ...
        {
            okButton.setText("Sort All Tracks");
            getContentPane().add(okButton, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 240, 130, -1));

            singleButton.setText("Sort Track");
            getContentPane().add(singleButton, new org.netbeans.lib.awtextra.AbsoluteConstraints(150, 240, 90, -1));

            cancelButton.setText("Cancel");
            getContentPane().add(cancelButton, new org.netbeans.lib.awtextra.AbsoluteConstraints(260, 240, 90, -1));
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new SortDialog(new javax.swing.JFrame(), true, null, false).setVisible(true);
            }
        });
    }
	...
	public void setSelections(String[] attributeNames, boolean[] ascending) 
	{
		if ( attributeNames.length > 0 )
			sortByComboBox.setSelectedItem(attributeNames[0]);
		if ( attributeNames.length > 1 )
			thenBy1ComboBox.setSelectedItem(attributeNames[1]);
		if ( attributeNames.length > 2 )
			thenBy2ComboBox.setSelectedItem(attributeNames[2]);
		
		if ( ascending.length > 0 )
			(ascending[0] ? acsendingRadioButton1 : descendingRadioButton1).setSelected(true);
		if ( ascending.length > 1 )
			(ascending[1] ? acsendingRadioButton2 : descendingRadioButton2).setSelected(true);
		if ( ascending.length > 2 )
			(ascending[2] ? acsendingRadioButton3 : descendingRadioButton3).setSelected(true);
	}

	public boolean isSortSingleTrack() {
		return sortSingleTrack;
	}
}
