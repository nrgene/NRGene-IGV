...
public class ReferenceFrame {
    ...
    private static boolean blockNegativeOrigin = true;
    ...
    private boolean blockInvalidChromosomeMessage = true;
    ...
    public void computeMaxZoom() {
        ...
        if (genome != null && chrName != null && genome.getChromosome(chrName) != null) {
            if (chrName.equals(Globals.CHR_ALL)) {
                ...
                maxZoom += (genome.getDecimalPlaces() * 3);
                if ( maxZoom < 0 )
                	maxZoom = 0;
    ...
    public void jumpTo(String chr, int start, int end) {
        ...
        if (chr != null) {
            if (genome.getChromosome(chr) == null && !chr.contains(Globals.CHR_ALL)) {
            	if ( !SessionReader.isCurrentlyLoading() && !blockInvalidChromosomeMessage ) 
    ...
    public void computeLocationScale() {
        ...
        if (genome != null) {
            ...
            if (setEnd > 0) {
                ...
                if ( genome.isGeneticMap() && chrName.equalsIgnoreCase("All") )
                {
                	setLocationScale(genome.getLength() / 1000.0 / nPixel);
                }
                else
                {
