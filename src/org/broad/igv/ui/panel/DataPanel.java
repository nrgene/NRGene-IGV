...
public class DataPanel extends JComponent implements Paintable {
    ...
    private String getLogTrack()
    {
        Collection<TrackGroup> groups = parent.getTrackGroups();
        if ( groups.size() > 0 )
        {
        	TrackGroup		group = groups.iterator().next();
        	
        	if ( group.size() > 0 )
        	{
        		Track 		track = group.getTracks().get(0);
    ...
    public void paintComponent(final Graphics g) {
    	...
    	Component						cursorComponent = this;
    	Cursor							cursor = cursorComponent.getCursor();
    	cursorComponent.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
    	...
    	*/
    	...
    	try
    	{
	    	...
	    	log.debug("start painting: " + getLogTrack());
	        ...
	        {
	        	log.error("exception (ignored): " + e.getMessage());
	        	e.printStackTrace();
	        } finally {
	        ...
	        log.debug("done painting: " + getLogTrack() + " (" + (System.currentTimeMillis() - started) + "ms)");
    	}
    	finally
    	{
			try
    ...
    private void computeMousableRegions(Collection<TrackGroup> groups, int width) {
        ...
        for (Iterator<TrackGroup> groupIter = groups.iterator(); groupIter.hasNext(); ) {
            ...
            if (group.isVisible()) {
                ...
                List<Track> trackList = group.getTracks(true);
     ...
     */
    static DecimalFormat locationFormatter_obs = new DecimalFormat();
    ...
    public void updateTooltipText(int x, int y, String positionString) {
    	...
    	log.debug("x=" + x + ", y=" + y + ", positionString=" + positionString);
        ...
        if (popupTextBuffer.length() > 6) {   // 6 characters for <html>
            ...
            if (!puText.equals(tooltipText)) {
            	...
            	if ( !PreferenceManager.getInstance().getAsBoolean(PreferenceManager.TOOLTIP_USE_PANEL))
            		setToolTipText(puText);
            	else
            	{
            		DecimalFormat		format = IGV.getInstance().getGenomeManager().currentGenome.getLocationFormatter();
            		
            		//IGV.getInstance().setStatusBarMessage2("Location: " + locationFormatter.format((int) displayLocation));
            		IGV.getInstance().setStatusBarMessage2("Location: " + format.format(displayLocation));

                	if ( popupTextBuffer.toString().indexOf("//hide//") < 0 )
                	{
            	    	popupTextBuffer.append("<br>Location: ");
            	        //popupTextBuffer.append(locationFormatter.format((int) displayLocation));
            	    	popupTextBuffer.append(IGV.getInstance().getGenomeManager().currentGenome.getLocationFormatter().format(displayLocation));
    ...
    class DataPanelMouseAdapter extends MouseInputAdapter {
        ...
        public void mouseMoved(MouseEvent e) {
            ...
            if (!frame.getChrName().equals(Globals.CHR_ALL)) {
                ...
                double location = frame.getChromosomePosition(e.getX()) + 1;
        		DecimalFormat		format = (IGV.getInstance().getGenomeManager().currentGenome != null)
        										? IGV.getInstance().getGenomeManager().currentGenome.getLocationFormatter()
        										: new DecimalFormat();
        		position = frame.getChrName() + ":" + format.format(location);
