...
abstract public class TrackPanelComponent extends JPanel {
    ...
    protected void openPopupMenu(TrackClickEvent te, List<Component> extraItems) {
        ...
        // Add saveImage
        if ( !TrackMenuUtils.isOnlyEmpty(selectedTracks) )
        {
	        menu.addSeparator();
    ...
    protected void toggleTrackSelections(MouseEvent e) {
        for (MouseableRegion mouseRegion : mouseRegions) {
            if (mouseRegion.containsPoint(e.getX(), e.getY())) {
                IGV.getInstance().getTrackManager().toggleTrackSelections(mouseRegion.getTracks(), e);
                return;
            }
        }
    }
    ...
    protected void clearTrackSelections() {
        ...
        for ( Track track : IGV.getInstance().getTrackManager().getAllTracks(false) )
        	if ( track instanceof VariantTrack )
        		((VariantTrack)track).clearSelectedSamples();
    ...
    protected void selectTracks(MouseEvent e) {
        ...
        for (MouseableRegion mouseRegion : mouseRegions) {
            if (mouseRegion.containsPoint(e.getX(), e.getY())) {
                ...
                IGV.getInstance().getTrackManager().setTrackSelections(mouseRegion.getTracks(), e);
