...
public class TrackPanel extends IGVPanel {
    ...
    public List<Track> getTracks() {
        ...
        for (TrackGroup tg : trackGroups) {
            tracks.addAll(tg.getTracks(true));
        }
        return tracks;
    }
    ...
    public void sortTracksByAttributes(final String attributeNames[], final boolean[] ascending,  boolean sortVariants) {

        assert attributeNames.length == ascending.length;

        for (TrackGroup tg : trackGroups) {
            tg.sortByAttributes(attributeNames, ascending, sortVariants);
        }
    }
    ...
    public void moveSelectedTracksTo(Collection<Track> selectedTracks,
    	...
    	// enforce having at least one group! (somehow gets erased when loading a session)
    	if ( trackGroups.size() == 0 )
    	{
    		log.debug("moveSelectedTracksTo: " + this.getName() + " adding null TrackGroup to avoid have none");
