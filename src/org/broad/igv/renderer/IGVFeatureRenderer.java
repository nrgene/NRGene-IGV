...
import org.broad.igv.track.ITrackBlockHeight;
...
public class IGVFeatureRenderer extends FeatureRenderer {
    ...
    public void render(List<IGVFeature> featureList,
        ...
        int							computedBlockHeight = 0, computedThinBlockHeight = 0;
        if ( track instanceof ITrackBlockHeight )
        {
        	int		height = ((ITrackBlockHeight)track).getTrackBlockHeight();
        	if ( height > 0 )
        	{
        		computedBlockHeight = height;
        		computedThinBlockHeight = (int)(height * ((float)THIN_BLOCK_HEIGHT / BLOCK_HEIGHT));
        	}
        }
        if ( computedBlockHeight <= 0 )
        {
        	computedBlockHeight = BLOCK_HEIGHT;
        	computedThinBlockHeight = THIN_BLOCK_HEIGHT;
        }
        
        blockHeight = displayMode == Track.DisplayMode.SQUISHED ? computedBlockHeight / 2 : computedBlockHeight;
        thinBlockHeight = displayMode == Track.DisplayMode.SQUISHED ? computedThinBlockHeight / 2 : computedThinBlockHeight;
        ...
        // fine tune for genetic view
        int			fineTune = (IGV.getInstance().getGenomeManager().currentGenome.isGeneticMap()) ? -1 : 0;
        ...
        if ((featureList != null) && (featureList.size() > 0)) {
            ...
            for (int i = 0; i < featureArray.length; i++) {
                ...
                double virtualPixelStart = Math.round((feature.getStart() + feature.getStartFraction() - origin + fineTune) / locScale);
                double virtualPixelEnd = Math.round((feature.getEnd() + feature.getEndFraction() - origin + fineTune) / locScale);
                ...
                if ((virtualPixelEnd >= trackRectangleX) && (virtualPixelStart <= trackRectangleMaxX)) {
                    ...
                    if (feature instanceof BasicFeature) {
                        ...
                        pixelThickStart = (int) Math.max(trackRectangleX, (bf.getThickStart() + bf.getStartFraction2() - origin + fineTune) / locScale);
                        pixelThickEnd = (int) Math.min(trackRectangleMaxX, (bf.getThickEnd() + bf.getEndFraction2() - origin + fineTune) / locScale);
                    }
                        ...
                        if (name != null) {
                            ...
                            if (verticalSpaceRequiredForText <= trackRectangle.height) {
                                ...
                                lastFeatureEndedAtPixelX = drawFeatureName(feature, nameStart, nameEnd,
                                        lastFeatureEndedAtPixelX, fontGraphics, textBaselineY, locScale, origin);
                            }
                        }
                    }
    ...
    protected void drawExons(IGVFeature gene, int yOffset, RenderContext context,
        ...
        for (Exon exon : gene.getExons()) {
            ...
            if ((pEnd >= trackRectangle.getX()) && (pStart <= trackRectangle.getMaxX())) {
                ...
                Graphics2D		fillGraphics = (exon.getColor() != null) ? context.getGraphic2DForColor(exon.getColor()) : blockGraphics;
                ...
                if (exon.isUTR()) {
                    ...
                    fillGraphics.fillRect(pClippedStart, yOffset - NON_CODING_HEIGHT / 2, pClippedWidth, NON_CODING_HEIGHT);
                    ...
                    if ((exon.getCdStart() < exon.getEnd()) && (exon.getCdEnd() > exon.getStart())) {
                        ...
                        fillGraphics.fillRect(pClippedStart, yOffset - blockHeight / 2, pClippedWidth, blockHeight);
    ...
    final private int drawFeatureName(IGVFeature feature, int pixelStart, int pixelEnd,
                                      ...
                                      int textBaselineY, double locScale, double origin) {
        ...
        if (nameStart > (lastFeatureEndedAtPixelX + fontSize)) {
            ...
            {
            	((AbstractFeature)feature).setLastDrawNameStart((int)(nameStart * locScale + origin));
            	((AbstractFeature)feature).setLastDrawNameWidth((int)(nameWidth * locScale));
            }
                                ...
                                int yOffset, Exon exon, Rectangle trackRectangle) 
    {
        ...
        if ((aaSequence != null) && aaSequence.hasNonNullSequence()) {
            ...
            int firstFullAcidIndex = (exon.getStrand() == Strand.POSITIVE) 		// DK: ???
            		? (int) Math.floor((aaSeqStartPosition - exon.getReadingShift()) / 3)
            				: aaSeqStartPosition;
                ...
                if (acid != null) {
                    ...
                    if ((px <= trackRectangle.getMaxX()) && (px2 >= trackRectangle.getX())) {
                        ...
                        if (((acid.getSymbol() == 'M') && (((gene.getStrand() == Strand.POSITIVE) &&
                                ...
                                (aaSeqStartPosition == exon.getCdEnd() - 3 + 1))))) {
            ...
            if (aaSeqStartPosition < exon.getEnd()) {
                ...
                aaRect.width = getPixelFromChromosomeLocation(exon.getChr(), cdEnd + 1, theOrigin, locationScale) - aaRect.x; // DK: added +1 since the end coor is inclusive
                Graphics2D bgGraphics = context.getGraphic2DForColor(odd ? AA_COLOR_1 : AA_COLOR_2);
                odd = !odd;
                bgGraphics.fill(aaRect);
            }
